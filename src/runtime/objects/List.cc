...
std::vector<Value> &List::values() {
    return _values;
}

Value List::operator[](int64_t index) const {
...
Value List::operator[](const Range &range) const {
    ...
    if (range.start().has_value()) {
        ...
        if (start < _values.begin()) start = _values.begin();
        if (start > _values.end()) start = _values.end() - 1;
    }
    if (range.end().has_value()) {
        end = _values.begin() + range.end().value() + (range.closed() ? 1 : 0);
        if (end < _values.begin()) end = _values.begin();
        if (end > _values.end()) end = _values.end();
    }
