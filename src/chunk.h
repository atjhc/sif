...
struct chunk : public base_chunk {	
	chunk(type type, size_t location, std::string &source)
		: base_chunk(type, source) {
		_seek(location);
	}

	chunk(type type, size_t location, const base_chunk &source)
		: base_chunk(type, source) {
		_seek(location);
	}
...
struct last_chunk : public base_chunk {		
	last_chunk(type type, std::string &source)
		: base_chunk(type, source) {
		_seek();
	}

	last_chunk(type type, const base_chunk &source)
		: base_chunk(type, source) {
		_seek();
	}
...
struct middle_chunk : public base_chunk {		
	middle_chunk(type type, std::string &source)
		: base_chunk(type, source) {
		_seek();
	}

	middle_chunk(type type, const base_chunk &source)
		: base_chunk(type, source) {
		_seek();
	}
